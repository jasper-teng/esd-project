<template>
  <div class="page">
    <div class="page-header teal">
      ...
      <div>
        <h1>Tap In</h1>
        <p>Start your journey — enter your card and boarding station</p>
      </div>
    </div>
    ...
    <div class="form-card">
      ...
      <button class="btn btn--teal" :disabled="!form.cardId.trim() || !form.station || loading" @click="handleTapIn">
        {{ loading ? 'Processing...' : 'Tap in now' }}
      </button>
    </div>
      ...
      <div v-if="result" class="result-panel" :class="result.type">
        ...
        <div class="result-body">
          ...
          <div v-if="result.incompleteSettled" class="sub-alert sub-alert--yellow">
            Unsettled trip detected — maximum fare of <strong>${{ result.incompleteSettled }}</strong> deducted and trip settled automatically.
          </div>
          <div v-if="result.autoTopUp" class="sub-alert sub-alert--purple">
            Balance was below $5.00 — Auto Top-Up of <strong>${{ result.autoTopUp }}</strong> triggered via your linked bank account.
          </div>
        </div>
      </div>
    </transition>
...
async function handleTapIn() {
  ...
  if (!c) {
    result.value = { type: 'error', title: 'Card not found', message: `No card found with ID "${form.value.cardId}". Try EZ-1234567890, EZ-LOW, or EZ-INCOMPLETE.` }
    loading.value = false; return
  }
  if (c.status !== 'active') {
    result.value = { type: 'error', title: 'Card unavailable', message: `This card is currently marked as "${c.status}" and cannot be used for travel.` }
    loading.value = false; return
  }
  ...
  if (balance < 5) {
    if (c.auto_topup) { balance += 10; autoTopUp = '10.00' }
      ...
      result.value = {
        ...
        message: 'Balance is below the $5.00 minimum required to board. Auto top-up is not enabled.',
        details: { 'Card holder': c.name, 'Current balance': `$${balance.toFixed(2)}`, 'Minimum required': '$5.00' },
        incompleteSettled,
      }
      loading.value = false; return
    }
  ...
  result.value = {
    type: 'success', title: 'Tap-in successful',
    message: `Welcome aboard, ${c.name}.`,
    details: {
      'Card': form.value.cardId,
