<template>
  <div class="auth-wrap">
    <div class="auth-card">
      ...
      <p class="auth-sub">FunFare – EZ-Link Portal</p>

      <form class="auth-form" @submit.prevent="handleRegister">
        <div class="field">
          <label class="field-label">Full Name</label>
          <input v-model="name" type="text" class="field-input" placeholder="John Doe" />
        </div>
        <div class="field">
        ...
        <div class="field">
          ...
          <input v-model="password" type="password" class="field-input" placeholder="Min. 6 characters" required minlength="6" />
        </div>
...
async function handleRegister() {
  ...
  try {
    ...
    if (!res.ok) {
      error.value = json.message || 'Registration failed'
      return
    }
...
.alert--success { background: #eaf7f0; border: 1px solid #b2e4cd; color: #1a6641; }
