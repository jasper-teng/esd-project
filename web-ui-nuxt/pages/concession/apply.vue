<template>
  <div class="page">
    <div class="page-header concession-header">
      <div class="page-icon">
    ...
    <div v-if="phase === 1" class="form-card">
      ...
      <button class="btn btn--lilac" :disabled="!canVerify || verifying" @click="handleVerify">
    ...
    <div v-if="phase === 2" class="form-card">
      ...
      <div class="form-actions">
        ...
        <button class="btn btn--lilac" :disabled="!canSubmit || submitting" @click="handleSubmit">
      ...
      <div v-if="phase === 3" class="form-card pending-card">
        ...
        <div class="timeline">
          <div class="tl-item tl-item--done">
            ...
            <div>
              <div class="tl-label">Student verification passed</div>
              <div class="tl-time">{{ verifiedAt }}</div>
            </div>
          </div>
          <div class="tl-item tl-item--done">
            <div class="tl-dot tl-dot--done">
            ...
            <div>
              <div class="tl-label">Application submitted & payment processed ($2.00)</div>
          ...
          <div class="tl-item" :class="processingDone ? 'tl-item--done' : 'tl-item--active'">
            ...
            <div>
              <div class="tl-label">Notification sent & concession card created</div>
              <div class="tl-time">{{ processingDone ? processedAt : 'Processing...' }}</div>
            </div>
          </div>
        </div>

        <transition name="fade">
          <div v-if="processingDone && approved" class="result-panel lilac-panel">
            <div class="result-icon lilac-icon">
        ...
        <transition name="fade">
          <div v-if="processingDone && !approved" class="result-panel error">
...
const canSubmit = computed(() =>
  form.value.cardId.trim() && form.value.contact.trim() &&
  form.value.email.trim() && form.value.postalCode.trim() &&
  form.value.unitNumber.trim() && form.value.document && form.value.paymentMethod
)
...
function reset() {
  phase.value          = 1
  verify.value         = { idNumber: '', school: '', dob: '' }
  form.value           = { cardId: '', contact: '', email: '', postalCode: '', unitNumber: '', document: '', paymentMethod: '' }
  verifyError.value    = ''
  submitError.value    = ''
...
.concession-header .page-icon { background: #e8e4f8; }
.concession-header .page-icon svg { stroke: #7c6fcd; }
.concession-header h1 { font-size: 18px; font-weight: 600; color: var(--text); letter-spacing: -0.2px; }
.concession-header p  { font-size: 13px; color: var(--muted); margin-top: 2px; }

/* ── Phase label ── */
.phase-label { display: flex; align-items: center; gap: 10px; }
.phase-badge {
  font-size: 11px; font-weight: 600; padding: 3px 9px;
  border-radius: 20px; background: #e8e4f8; color: #7c6fcd;
}
.phase-badge--done { background: #d4f7e8; color: #1a6641; }
.phase-title { font-size: 15px; font-weight: 600; color: var(--text); }

/* ── Verified banner ── */
.verified-banner {
  display: flex; align-items: center; gap: 8px;
  background: #d4f7e8; border: 1px solid #9de0b8;
  border-radius: 8px; padding: 10px 14px;
  font-size: 13px; color: #1a6641;
}
...
.file-upload:hover { border-color: #7c6fcd; color: #7c6fcd; }
.file-upload.has-file { border-color: #7c6fcd; border-style: solid; color: #4a3bbf; }
...
.payment-option {
  ...
  padding: 10px 12px; border: 1.5px solid var(--border); border-radius: var(--rs);
  cursor: pointer; font-size: 13px; font-weight: 500; color: var(--muted);
  transition: all 0.15s; background: var(--surface);
}
.payment-option:hover { border-color: #7c6fcd; color: #7c6fcd; }
.payment-option.selected { border-color: #7c6fcd; background: #f0eefb; color: #4a3bbf; font-weight: 600; }
...
.btn--lilac {
  ...
  padding: 10px 18px; border: none; border-radius: var(--rs);
  font-family: var(--font); font-size: 13px; font-weight: 600;
  cursor: pointer; transition: background 0.15s, transform 0.1s;
}
.btn--lilac:hover:not(:disabled) { background: #6a5cbd; transform: translateY(-1px); }
.btn--lilac:disabled { opacity: 0.4; cursor: not-allowed; }
...
/* ── Alert ── */
.alert { display: flex; align-items: center; gap: 8px; border-radius: var(--rs); padding: 10px 14px; font-size: 13px; font-weight: 500; }
...
.tl-item--active  { border-left-color: #b8aee8; }
...
.tl-dot--active { background: #b8aee8; border-color: #b8aee8; }
...
.station-input.open  { border-color: #7c6fcd; }
